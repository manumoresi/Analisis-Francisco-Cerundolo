...
g1=ggplot(cerundolo_final1) +
  ...
  geom_point(data = subset(cerundolo_final1, fran_win == levels(cerundolo_final1$fran_win)[1]), 
             aes(x = median(w, na.rm=TRUE), y = 0), color = "blue", size = 3) +
  ...
  geom_point(data = subset(cerundolo_final1, fran_win == levels(cerundolo_final1$fran_win)[2]), 
             aes(x = median(l, na.rm=TRUE), y = 0), color = "blue", size = 3)+
  scale_fill_manual(values = c("Perdió"="steelblue1","Ganó"="steelblue4"))
...
modelo_final_2 = glm(fran_win1 ~ opp_age + pct_fran_1stIn + pct_opp_1stIn + pct_fran_1stWon + pct_opp_1stWon + pct_fran_2ndWon + pct_opp_2ndWon + pct_fran_bp_wins + pct_opp_bp_wins, data=cerundolo_modelo,family = binomial(link="logit"))

summary(modelo_final_2)

# Test de Hosmer-Lemeshow usando hltest de glmtoolbox
hltest(modelo_final)
hltest(modelo_final_2)
...
```{r, echo=FALSE, warning=FALSE, message=FALSE, results='hide'}
modelo_cuasi_final = glm(fran_win1 ~ pct_fran_1stIn + pct_opp_1stIn + pct_fran_1stWon + pct_opp_1stWon + pct_fran_2ndWon + pct_opp_2ndWon + pct_fran_bp_wins + pct_opp_bp_wins, data=cerundolo_modelo,family = binomial(link="logit"))

anova(modelo_cuasi_final, modelo_final_2, test="LRT")

#Podríamos sacarla del modelo

modelo_final_2 = glm(fran_win1 ~ pct_fran_1stIn + pct_opp_1stIn + pct_fran_1stWon + pct_opp_1stWon + pct_fran_2ndWon + pct_opp_2ndWon + pct_fran_bp_wins + pct_opp_bp_wins, data=cerundolo_modelo,family = binomial(link="logit"))
```
